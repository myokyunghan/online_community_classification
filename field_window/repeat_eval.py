...
from compare_with_golden import aggregate_predictions, load_rows  # noqa: E402
from compare_with_golden_fewshot import classify_row_multi, compare_row_field_window  # noqa: E402  (이 폴더의 버전 재사용)
...
def run_one_repeat(
    ...
    for i, row in enumerate(test_rows, 1):
        ...
        try:
            ...
            result = compare_row_field_window(row, predictions, aggregate)
        ...
        # golden엔 ENV 정답이 없고 이번 실험은 ROLE(FIELD/WINDOW) 판정만 채점 대상이므로,
        # env_relevant일 때 덤으로 붙는 ENV_* 코드는 precision/recall 계산에서 제외한다 —
        # 안 그러면 ROLE을 정확히 맞혀도 ENV가 붙었다는 이유로 precision이 깎여
        # exact_match(ROLE만 봄)와 f1(ENV까지 봄)이 서로 다른 걸 측정하게 된다.
        pred = {c for c in _code_set(result.get("majority_codes", "")) if c.startswith("ROLE_")}
        if is_err_gold:
