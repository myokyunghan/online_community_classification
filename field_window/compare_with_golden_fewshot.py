"""
classify_paper_fewshot.py(시스템프롬프트+few-shot, 요약 단계 없음)로 golden 30편을
분류하고 정답과 비교한다. 재시도/다수결 집계는 루트 compare_with_golden.py의 범용 로직을
재사용하지만, 정답 비교(compare_row_field_window)는 이 폴더 전용이다 — golden의 class1이
이제 11개 코드가 아니라 FIELD/WINDOW/ERR 이진 라벨이라, 루트의 compare_row()는 그 라벨을
인식하지 못한다.
...
from compare_with_golden import (  # noqa: E402  (루트의 범용 로직 재사용 — compare_row는 예전
    aggregate_predictions,  # 11개 코드 체계의 VALID_CODES 기준으로 라벨을 매핑해서 "FIELD"/
    classify_retry_kind,  # "WINDOW" 라벨을 인식 못 하므로 여기서는 재사용하지 않는다. 아래
    load_rows,  # compare_row_field_window()가 그 대체 역할을 한다.
)
...
def _predicted_labels_from_codes(predicted_codes: set) -> set:
    """predicted_codes(예: {"ROLE_FIELD"}, {"ENV_RELEVANT", "ROLE_WINDOW"})를 FIELD/WINDOW/
    ENV 라벨 집합으로 바꾼다. env_relevant와 role_relevant가 둘 다 true인 논문은 이제 둘 다
    호출되므로 라벨이 2개(예: {"ENV", "FIELD"})까지 나올 수 있다 — golden은 라벨 하나뿐이라,
    그 라벨이 이 집합 **안에 있으면** 정답으로 친다(멤버십 체크. 예전처럼 단일 선택 강제 아님)."""
    labels = set()
    if "ENV_RELEVANT" in predicted_codes:
        labels.add("ENV")
    role_code = next((c for c in predicted_codes if c in ("ROLE_FIELD", "ROLE_WINDOW")), None)
    if role_code:
        labels.add(role_code.replace("ROLE_", ""))
    return labels


def compare_row_field_window(row: dict, predictions: list[dict], aggregate: dict) -> dict:
    """golden class1이 FIELD/WINDOW/ENV/ERR 중 하나뿐인 3+1갈래 분류용 비교 함수.
    루트 compare_with_golden.compare_row()는 11개 코드 체계(VALID_CODES)를 기준으로 라벨을
    매핑하므로 이 문자열들을 그냥 버려버린다 — 그래서 이 폴더 전용으로 새로 짠다.
    반환하는 필드 이름·형태는 compare_row()와 동일하게 맞춰서, repeat_eval.py 등 하위 코드는
    손대지 않고 그대로 재사용할 수 있게 한다."""
    class1_raw = row.get("class1", "").strip()
    gold_choice = class1_raw.upper()
    is_err_gold = gold_choice == "ERR"
    ...
    predicted_labels = _predicted_labels_from_codes(predicted_codes)

    if is_err_gold:
        exact_match = aggregate["majority_status"] == "ERR"
        gold_covered = exact_match
        class1_code, gold_codes_display = "ERR", "ERR"
    else:
        # gold_choice가 FIELD/WINDOW/ENV가 아닌 값이면 모델이 애초에 낼 수 없는 선택지이므로
        # 항상 미스로 집계된다 — 크래시는 안 나되, 이런 논문이 있다면 raw 값을 확인해봐야 한다.
        # predicted_labels엔 라벨이 2개(예: {"ENV","FIELD"})까지 있을 수 있는데, gold가 그중
        # 하나에 포함되면 정답으로 친다 — 단일 선택 강제가 아니라 멤버십 체크다.
        exact_match = gold_choice in predicted_labels
        gold_covered = exact_match  # 정답이 하나뿐이라 "포함" 개념이 멤버십 체크와 같다
        class1_code = gold_choice
        gold_codes_display = "ENV_RELEVANT" if gold_choice == "ENV" else f"ROLE_{gold_choice}"

    gold_dropped_by_cap = (not is_err_gold) and any(gold_codes_display in d for d in dropped_runs)
...
def main():
    ...
    for i, row in enumerate(rows, 1):
        ...
        try:
            ...
            results.append(compare_row_field_window(row, predictions, aggregate))
